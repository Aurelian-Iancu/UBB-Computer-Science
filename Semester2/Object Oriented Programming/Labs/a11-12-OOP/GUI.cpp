...
service{serv}, validator{validator1}, repository{repo}{
    ...
    this->movieListWidget->setStyleSheet("background-color: qlineargradient(x1: 0.5, y1: 0.5 x2: 0.5, y2: 1, stop: 0 #6666FF, stop: 0.7 #99CCFF );");
...
int DogListModel::rowCount(const QModelIndex &parent) const {
    return this->repository.getRepo().size();
}

QVariant DogListModel::data(const QModelIndex &index, int role) const {
    int row = index.row();
    Movie movie = this->repository.getRepo()[row];
