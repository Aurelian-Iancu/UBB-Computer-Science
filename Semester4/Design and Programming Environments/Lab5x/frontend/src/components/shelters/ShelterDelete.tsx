...
export const ShelterDelete = () => {
    ...
    useEffect(() => {
        const fetchShelter = async () => {
            const response = await fetch(`${BACKEND_API_URL}/shelter/delete/${shelterId}`);
    ...
    const handleDelete = async (event: { preventDefault: () => void }) => {
        ...
        try {
            await axios.delete(`${BACKEND_API_URL}/shelter/delete/${shelterId}`);
            navigate("/shelter");
        } catch (error: any) {
            console.error(error.message);
            setErrorMessage("Shelter could not be deleted. ");
