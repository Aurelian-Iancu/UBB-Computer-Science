...
import { ShelterOrdAvgWeight } from "../../models/ShelterOrdAvgWeight";

export const ShelterStatisticsAverageWeight = () => {
    const[loading, setLoading] = useState(true);
    const[shelters, setShelters] = useState([]);

    useEffect(() => {
        fetch(`${BACKEND_API_URL}/shelter/statistics/averageWeight`)
            .then(res => res.json())
    ...
    return (
        ...
        <Container>
            <h1 style={{marginTop:"65px"}}>Shelters ordered by average weight</h1>
                    ...
                    <Table sx={{ minWidth: 800 }} aria-label="simple table" style={{backgroundColor:"whitesmoke"}}>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell align="center" style={{color:"#2471A3", fontWeight:'bold'}}>Average weight</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {shelters.map((shelter:ShelterOrdAvgWeight, index) => (
                                <TableRow key={shelter.shelterName}>
                                    <TableCell component="th" scope="row">
                                        {index + 1}
                                    </TableCell>
                                    <TableCell align="center">{shelter.shelterName}</TableCell>
                                    <TableCell align="center">{shelter.average}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
